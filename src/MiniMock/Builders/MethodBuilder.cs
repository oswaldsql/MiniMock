    ...
    private static void Build(CodeBuilder builder, IMethodSymbol symbol, List<MethodSignature> helpers, int methodCount)
    {
        ...
        if(typeList != "" && !parameterList.Contains("out "))//  && !parameterList.Contains("ref "))
        {
            ...
            builder.Add($$"""
                          ...
                          {
                              if(!this.{{functionPointer}}_Assert2({{nameList}})) throw new System.ArgumentException("One or more arguments did not match the specified assertions for {{symbol}}.");
                          ...
                          private {{functionPointer}}_Assert {{functionPointer}}_Assert2 {get;set;} = ({{parameterList}}) => true;
                          ...
                          """);
            helpers.Add(new($$"""{{functionPointer}}_Assert assert""", $$"""this.target.{{functionPointer}}_Assert2 = assert;""", Documentation.CallBack));
        }
        else
        builder.Add($$"""
                      public delegate {{methodReturnType}} {{functionPointer}}_Delegate({{parameterList}});
        ...
        if (!HasOutOrRef(symbol) && !symbol.ReturnsVoid)
        {
            ...
            var code = $$"""
                            ...
                            """;
            helpers.Add(new($"System.Collections.Generic.IEnumerable<{methodReturnType}> returnValues", code, Documentation.SpecificValueList, seeCref));
        }
