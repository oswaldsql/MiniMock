...
public class Demo(ITestOutputHelper testOutputHelper)
{
    ...
    public async Task TestingAllTheOptions()
    {
        ...
        var versionLibrary = Mock.IVersionLibrary(config => config
                ...
                .DownloadExists(assert: version => !string.IsNullOrWhiteSpace(version)) // Returns true if the version is not null or empty
                ...
                .DownloadLinkAsync(assert:version => !string.IsNullOrWhiteSpace(version)) // Throws a assertion exception if the version is null or empty
